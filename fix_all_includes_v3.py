...
TEAKRA_HDR_ROOT = os.path.join(REPO_ROOT, 'src/core/melonds/teakra/include')
...
def process_file(file_path):
    ...
    # Headers to redirect
    targets = {
        'teakra.h': 'teakra/teakra.h',
        'teakra_c.h': 'teakra/teakra_c.h',
        'disassembler.h': 'teakra/disassembler.h',
        'disassembler_c.h': 'teakra/disassembler_c.h'
    }
        ...
        if b'#include' in line:
            ...
            if m:
                ...
                if filename in targets:
                    ...
                    hdr_rel_path = targets[filename]
                    hdr_abs = os.path.join(TEAKRA_HDR_ROOT, hdr_rel_path)
                    ...
                    if path_str != rel:
                        # Replace the entire include path with the correct relative one
                        # and force using double quotes
                        new_line = line.replace(m.group(0), b'#include "' + rel.encode() + b'"')
                        changed = True

        # C API Fixes (Removing non-existent member)
    ...
    # Post-process teakra_c.cpp to remove the function body
