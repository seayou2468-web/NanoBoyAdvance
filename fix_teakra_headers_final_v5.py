...
def process_file(file_path):
    ...
    for line in lines:
        ...
        if b'#include' in line:
            ...
            # This time we look for anything that contains 'teakra/' AND one of the targets
            for target in targets:
                if b'teakra/' + target.encode() in line:
                    # Found it. Calculate relative path.
                    hdr_abs = os.path.join(TEAKRA_HDR_ROOT, target)
                    file_dir_abs = os.path.abspath(os.path.dirname(file_path))
                    rel = os.path.relpath(hdr_abs, file_dir_abs).replace('\\', '/')

                    # Identify the whole include path to replace
                    # It's either inside "" or <>
                    import re
                    ...
                    if m:
                        ...
                        if old_path != rel.encode():
                            new_line = line.replace(old_path, rel.encode())
                            # Force ""
