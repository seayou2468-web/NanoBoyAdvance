...
std::optional<std::string> CanonicalSysdataNameForInputPath(const std::filesystem::path& path) {
  ...
  if (name.find("firmware") != std::string::npos || name.find("firm") != std::string::npos) {
    return std::string("firmware.bin");
  }
  return std::nullopt;
}
...
bool LoadRomFromPath(void* opaque_runtime, const char* rom_path, std::string& last_error) {
  ...
  try {
    if (!runtime->emulator->loadROM(fs_path)) {
      last_error =
          "Mikage failed to load 3DS ROM. If this title is encrypted, ensure sysdata/aes_keys.txt "
          "(and optional sysdata/seeddb.bin) exist in the emulator app data directory.";
      return false;
    }
