...
std::string GetSystemSaveDataPath(std::string_view mount_point, const Path& path) {
    ...
    return StringFromFormat("%s%08X/%08X/", std::string(mount_point).c_str(), save_low, save_high);
}

std::string GetSystemSaveDataContainerPath(std::string_view mount_point) {
    return StringFromFormat("%sdata/%s/sysdata/", std::string(mount_point).c_str(), SYSTEM_ID);
}
