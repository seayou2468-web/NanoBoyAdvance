 ...
 * a real 3ds.
 */
constexpr Common::Crypto::AESBlock nwm_beacon_key = {};
...
std::vector<u8> GenerateNintendoNetworkInfoTag(const NetworkInfo& network_info) {
    ...
    std::array<u8, CC_SHA1_DIGEST_LENGTH> hash{};
    CC_SHA1(buffer.data() + offsetof(NetworkInfoTag, network_info),
            static_cast<CC_LONG>(buffer.size() - sizeof(TagHeader)), hash.data());
 ...
 */
Common::Crypto::AESBlock GetBeaconCryptoCTR(const NetworkInfo& network_info) {
...
std::vector<u8> GeneratedEncryptedData(const NetworkInfo& network_info, const NodeList& nodes) {
    ...
    std::array<u8, CC_MD5_DIGEST_LENGTH> hash{};
    CC_MD5(buffer.data() + offsetof(BeaconData, bitmask),
           static_cast<CC_LONG>(buffer.size() - sizeof(data.md5_hash)), hash.data());
    ...
    const Common::Crypto::AESBlock counter = GetBeaconCryptoCTR(network_info);
    ASSERT_MSG(Common::Crypto::AESCTRTransform(
                   std::span<const u8>(buffer.data(), buffer.size()),
                   std::span<u8>(buffer.data(), buffer.size()), nwm_beacon_key, counter),
               "UDS beacon encryption failed");
