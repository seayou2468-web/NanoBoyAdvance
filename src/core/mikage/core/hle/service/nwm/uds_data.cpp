 ...
 */
static Common::Crypto::AESBlock GetDataCryptoCTR(const NetworkInfo& network_info) {
    ...
    CC_MD5(reinterpret_cast<u8*>(&data), static_cast<CC_LONG>(sizeof(data)), hash.data());
 ...
 */
[[maybe_unused]] static Common::Crypto::AESBlock GenerateDataCCMPKey(
    std::span<const u8> passphrase, const NetworkInfo& network_info) {
    // Calculate the MD5 hash of the input passphrase.
    Common::Crypto::AESBlock passphrase_hash{};
    CC_MD5(passphrase.data(), static_cast<CC_LONG>(passphrase.size()), passphrase_hash.data());

    Common::Crypto::AESBlock ccmp_key{};

    // The CCMP key is the result of encrypting the MD5 hash of the passphrase with AES-CTR using
    // keyslot 0x2D.
    const Common::Crypto::AESBlock counter = GetDataCryptoCTR(network_info);
    const Common::Crypto::AESBlock key = HW::AES::GetNormalKey(HW::AES::KeySlotID::UDSDataKey);
    if (!Common::Crypto::AESCTRTransform(passphrase_hash, ccmp_key, key, counter)) {
        LOG_ERROR(Service_NWM, "Failed to derive UDS data CCMP key");
    }
...
[[maybe_unused]] static std::vector<u8> DecryptDataFrame(
    std::span<const u8> encrypted_payload, const Common::Crypto::AESBlock& ccmp_key,
    ...
    (void)aad;
    (void)nonce;
    LOG_ERROR(Service_NWM, "DecryptDataFrame is unavailable without Crypto++ CCM support");
    return {};
}
...
[[maybe_unused]] static std::vector<u8> EncryptDataFrame(
    std::span<const u8> payload, const Common::Crypto::AESBlock& ccmp_key,
    const MacAddress& sender, const MacAddress& receiver, const MacAddress& bssid,
    u16 sequence_number, u16 frame_control) {
    // Reference: IEEE 802.11-2007
    ...
    (void)aad;
    (void)nonce;
    LOG_ERROR(Service_NWM, "EncryptDataFrame is unavailable without Crypto++ CCM support");
    return {};
}
