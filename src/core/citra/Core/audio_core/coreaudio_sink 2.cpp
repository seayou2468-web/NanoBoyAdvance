...
unsigned int CoreAudioSink::GetNativeSampleRate() const {
    unsigned int sr = native_sample_rate;
    UInt32 sr_size;
    AudioUnitGetProperty(audio_unit, kAudioUnitProperty_SampleRate, kAudioUnitScope_Input, 0, &sr, &sr_size);
    return sr; // TODO: this is stub
}
