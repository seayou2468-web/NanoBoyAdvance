...
static inline bool CondPassedOrAL(const ARMul_State* cpu, unsigned int cond) {
    if (cond == ConditionCode::AL) [[likely]] {
        return true;
    }
    return CondPassed(cpu, cond);
}
...
ADC_INST: {
    if (CondPassedOrAL(cpu, inst_base->cond)) {
