...
namespace Crypto {
	...
	class AESEngine {
	  ...
	  public:
		...
		constexpr bool hasKeyX(usize slotId) { return slotId < AesKeySlotCount && m_slots.at(slotId).keyX.has_value(); }
		constexpr AESKey getKeyX(usize slotId) { return m_slots.at(slotId).keyX.value_or(AESKey{}); }
		constexpr void setKeyX(usize slotId, const AESKey& key) { if (slotId < AesKeySlotCount) { m_slots.at(slotId).keyX = key; updateNormalKey(slotId); } }
		constexpr bool hasKeyY(usize slotId) { return slotId < AesKeySlotCount && m_slots.at(slotId).keyY.has_value(); }
		constexpr AESKey getKeyY(usize slotId) { return m_slots.at(slotId).keyY.value_or(AESKey{}); }
		constexpr void setKeyY(usize slotId, const AESKey& key) { if (slotId < AesKeySlotCount) { m_slots.at(slotId).keyY = key; updateNormalKey(slotId); } }
		constexpr bool hasNormalKey(usize slotId) { return slotId < AesKeySlotCount && m_slots.at(slotId).normalKey.has_value(); }
		constexpr AESKey getNormalKey(usize slotId) { return m_slots.at(slotId).normalKey.value_or(AESKey{}); }
		constexpr void setNormalKey(usize slotId, const AESKey& key) { if (slotId < AesKeySlotCount) { m_slots.at(slotId).normalKey = key; } }
