 ...
 */
static std::array<u8, 16> GetDataCryptoCTR(
    ...
    Common::CryptoUtil::Md5Digest(reinterpret_cast<u8*>(&data), sizeof(data), hash.data());
 ...
 */
[[maybe_unused]] static std::array<u8, 16> GenerateDataCCMPKey(
    std::span<const u8> passphrase, const NetworkInfo& network_info) {
    // Calculate the MD5 hash of the input passphrase.
    std::array<u8, 16> passphrase_hash;
    Common::CryptoUtil::Md5Digest(passphrase.data(), passphrase.size(), passphrase_hash.data());

    std::array<u8, 16> ccmp_key;

    // The CCMP key is the result of encrypting the MD5 hash of the passphrase with AES-CTR using
    // keyslot 0x2D.
    std::array<u8, 16> counter = GetDataCryptoCTR(network_info);
    std::array<u8, 16> key = HW::AES::GetNormalKey(HW::AES::KeySlotID::UDSDataKey);
    Common::AESUtil::AesCtrCryptor aes;
    aes.SetKeyWithIV(key.data(), key.size(), counter.data());
...
[[maybe_unused]] static std::vector<u8> DecryptDataFrame(
    std::span<const u8> encrypted_payload, const std::array<u8, 16>& ccmp_key,
...
#if defined(__APPLE__)
    const auto decrypted = Common::CryptoUtil::AesCcmDecrypt(
        ccmp_key, nonce, aad, encrypted_payload.subspan(0, encrypted_payload.size() - 8),
        encrypted_payload.subspan(encrypted_payload.size() - 8), false);
    if (!decrypted.empty() || encrypted_payload.size() == 8) {
        return decrypted;
    }
#else
    try {
...
[[maybe_unused]] static std::vector<u8> EncryptDataFrame(
    std::span<const u8> payload, const std::array<u8, 16>& ccmp_key,
    const MacAddress& sender, const MacAddress& receiver, const MacAddress& bssid,
    u16 sequence_number, u16 frame_control) {
    // Reference: IEEE 802.11-2007
...
#if defined(__APPLE__)
    return Common::CryptoUtil::AesCcmEncrypt(ccmp_key, nonce, aad, payload, 8, false);
#else
    try {
