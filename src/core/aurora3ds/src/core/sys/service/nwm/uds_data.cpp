 ...
 */
static std::array<u8, MD5DigestSize> GetDataCryptoCTR(const NetworkInfo& network_info) {
    ...
    std::array<u8, MD5DigestSize> hash;
#if defined(__APPLE__) && TARGET_OS_IPHONE
    CC_MD5(reinterpret_cast<const unsigned char*>(&data), sizeof(data), hash.data());
#else
 ...
 */
[[maybe_unused]] static std::array<u8, AESBlockSize> GenerateDataCCMPKey(
    std::span<const u8> passphrase, const NetworkInfo& network_info) {
    // Calculate the MD5 hash of the input passphrase.
    std::array<u8, MD5DigestSize> passphrase_hash;
#if defined(__APPLE__) && TARGET_OS_IPHONE
    CC_MD5(passphrase.data(), passphrase.size(), passphrase_hash.data());
#else
    CryptoPP::Weak::MD5().CalculateDigest(passphrase_hash.data(), passphrase.data(),
                                          passphrase.size());
#endif

    std::array<u8, AESBlockSize> ccmp_key{};

    // The CCMP key is the result of encrypting the MD5 hash of the passphrase with AES-CTR using
    // keyslot 0x2D.
#if defined(__APPLE__) && TARGET_OS_IPHONE
    // iOS build path: avoid Crypto++ dependency.
    // TODO: Replace with CommonCrypto AES-CTR equivalent if UDS encryption is enabled on iOS.
    ccmp_key = HW::AES::GetNormalKey(HW::AES::KeySlotID::UDSDataKey);
#else
    using CryptoPP::AES;
    std::array<u8, MD5DigestSize> counter = GetDataCryptoCTR(network_info);
...
[[maybe_unused]] static std::vector<u8> DecryptDataFrame(
    std::span<const u8> encrypted_payload, const std::array<u8, AESBlockSize>& ccmp_key,
    const MacAddress& sender, const MacAddress& receiver, const MacAddress& bssid,
    u16 sequence_number, u16 frame_control) {
#if defined(__APPLE__) && TARGET_OS_IPHONE
    LOG_ERROR(Service_NWM, "DecryptDataFrame is unavailable on iOS without Crypto++");
    return {};
#else
...
[[maybe_unused]] static std::vector<u8> EncryptDataFrame(
    std::span<const u8> payload, const std::array<u8, AESBlockSize>& ccmp_key,
    const MacAddress& sender, const MacAddress& receiver, const MacAddress& bssid,
    u16 sequence_number, u16 frame_control) {
#if defined(__APPLE__) && TARGET_OS_IPHONE
    LOG_ERROR(Service_NWM, "EncryptDataFrame is unavailable on iOS without Crypto++");
    return {};
#else
    // Reference: IEEE 802.11-2007
