...
SurfaceParams SurfaceParams::FromInterval(SurfaceInterval interval) const {
    ...
    PAddr aligned_start =
        start + Common::AlignDown(aurora::icl::first(interval) - start, stride_tiled_bytes);
    PAddr aligned_end =
        start + Common::AlignUp(aurora::icl::last_next(interval) - start, stride_tiled_bytes);
    ...
    if (aligned_end - aligned_start > stride_tiled_bytes) {
        ...
        aligned_start =
            start + Common::AlignDown(aurora::icl::first(interval) - start, tiled_alignment);
        aligned_end =
            start + Common::AlignUp(aurora::icl::last_next(interval) - start, tiled_alignment);
