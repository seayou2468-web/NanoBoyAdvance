...
- `gba`: existing GBA runtime.
- `gba/platform`: GBA-specific platform layer moved from `src/platform/core` so GBA-related
...
1. Create `src/core/<new-core>/` and add a runtime module similar to `gba/runtime.*`.
